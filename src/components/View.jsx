...
const View = ({addResponseFromHome,deleteResponseFromCategory,setDeleteResponseFromView}) => {
  const[deleteVideoFromVideoCard,setDeleteVideoFromVideoCard] = useState("")
  const[allVideos,setAllVideos] = useState([])

  useEffect(()=>{
    getAllVideos()
  },[addResponseFromHome,deleteVideoFromVideoCard,deleteResponseFromCategory])
      ...
      <Row droppable="true" onDragOver={dragOverView} onDrop={e=>categoryVideoDropOverView(e)}>
        {
          ...
          allVideos?.map(video=>(
            <Col className='mb-2' sm={12} md={6} lg={4}>
              <VideoCard setDeleteVideoFromVideoCard={setDeleteVideoFromVideoCard} displayData={video}/>
            </Col>
          ))
        :
